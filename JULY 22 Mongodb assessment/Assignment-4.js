...
const swati = db.trainers.findOne({ name: "Swati Nair" })
db.sessions.find({ trainer_id: swati.trainer_id })

// 5. Find all members who attended a session on 2024-08-05.
db.sessions.aggregate([{ $match: { date: new Date("2024-08-05") } },{$lookup: {from: "members",localField: "member_id",foreignField: "member_id",as: "member"}, { $unwind: "$member" },{ $project: { _id: 0, name: "$member.name", session_type: 1 } }])

// Intermediate Queries
// 6. Count the number of sessions each member has attended.
db.sessions.aggregate([{ $group: { _id: "$member_id", session_count: { $sum: 1 } } }])

// 7. Show average duration of sessions for each session_type.
db.sessions.aggregate([{ $group: { _id: "$session_type", avg_duration: { $avg: "$duration" } } }])

// 8. Find all female members who attended a session longer than 60 minutes.
db.sessions.aggregate([{ $match: { duration: { $gt: 60 } } },{$lookup: {from: "members",localField: "member_id",foreignField: "member_id",as: "member"}},{ $unwind: "$member" },{ $match: { "member.gender": "Female" } },{ $project: { _id: 0, name: "$member.name", duration: 1 } }])
...
db.sessions.aggregate([{$group: {_id: "$member_id",unique_trainers: { $addToSet: "$trainer_id" }}},{$project: {member_id: "$_id",trainer_count: { $size: "$unique_trainers" }}},{ $match: { trainer_count: { $gt: 1 } } }])

// Advanced Queries
// 11. Use $lookup to display sessions with member name and trainer name.
db.sessions.aggregate([{$lookup: {from: "members",localField: "member_id",foreignField: "member_id",as: "member"}},{ $unwind: "$member" },{$lookup: {from: "trainers",localField: "trainer_id",foreignField: "trainer_id",as: "trainer"}},{ $unwind: "$trainer" },{$project: {_id: 0,session_id: 1,member_name: "$member.name",trainer_name: "$trainer.name",session_type: 1,duration: 1}}])

// 12. Calculate total session time per trainer.
db.sessions.aggregate([{ $group: { _id: "$trainer_id", total_time: { $sum: "$duration" } } }])

// 13. List each member and their total time spent in the gym.
db.sessions.aggregate([{ $group: { _id: "$member_id", total_duration: { $sum: "$duration" } } }])

// 14. Count how many sessions each trainer has conducted.
db.sessions.aggregate([{ $group: { _id: "$trainer_id", session_count: { $sum: 1 } } }])

// 15. Find which trainer has conducted the longest average session duration.
db.sessions.aggregate([{ $group: { _id: "$trainer_id", avg_duration: { $avg: "$duration" } } },{ $sort: { avg_duration: -1 } },{ $limit: 1 }])

// 16. Show how many unique members each trainer has trained.
db.sessions.aggregate([{$group: {_id: "$trainer_id",unique_members: { $addToSet: "$member_id" }}},{$project: {trainer_id: "$_id",member_count: { $size: "$unique_members" }}}])

// 17. Find the most active member (by total session duration).
db.sessions.aggregate([{ $group: { _id: "$member_id", total_time: { $sum: "$duration" } } },{ $sort: { total_time: -1 } },{ $limit: 1 }])

// 18. List all Gold membership members who took at least one Strength session.
db.sessions.aggregate([{ $match: { session_type: "Strength" } },{$lookup: {from: "members",localField: "member_id",foreignField: "member_id",as: "member"}},{ $unwind: "$member" },{ $match: { "member.membership_type": "Gold" } },{ $group: { _id: "$member.member_id", name: { $first: "$member.name" } } }])

// 19. Display a breakdown of sessions by membership type.
db.sessions.aggregate([{$lookup: {from: "members",localField: "member_id",foreignField: "member_id",as: "member"}},{ $unwind: "$member" },{$group: {_id: "$member.membership_type",total_sessions: { $sum: 1 }}}])

// 20. Find members who have not attended any session.
